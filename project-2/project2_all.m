...
for experiment = 1:num_experiments
    ...
    x_oracle = oracle(A_eff, b, find(x0));
...
figure
subplot(131);
imshow(reshape(full(b0), n, n), [], 'InitialMagnification', 'fit')
title('Clean Image');

subplot(132)
imshow(reshape(full(b0_noisy), n, n), [], 'InitialMagnification', 'fit')
title('Noisy Image');

subplot(133)
imshow(reshape(full(C' * b), n, n), [], 'InitialMagnification', 'fit')
...
subplot(223)
imshow(reshape(full(b_oracle), n, n), [], 'InitialMagnification', 'fit')
title('Oracle reconstruction');
...
figure; semilogx(lambda_vec, psnr_admm_lambda, '-*r', 'LineWidth', 2);
...
subplot(223)
imshow(reshape(full(best_b_omp), n, n), [], 'InitialMagnification', 'fit')
...
subplot(223)
imshow(reshape(full(best_b_admm), n, n), [], 'InitialMagnification', 'fit')
...
figure; plot(p_vec, mse_omp_p, '-*r', 'LineWidth', 2);
...
figure; plot(sigma_vec, mse_omp_sigma, '-*r', 'LineWidth', 2);
